...
void Intro::Render()
{
	...
	RenderMeshOnScreen(meshList[0], 40.f, 35.f, 2.f, 2.f);
...
void Intro::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void Intro::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void Intro::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0)
		return;
...
void Intro::RenderMeshOnScreen(Mesh* mesh, float x, float y, float sizex, float sizey)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10);
