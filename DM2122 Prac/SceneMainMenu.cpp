...
void SceneMainMenu::Init()
{
	...
	//meshList[GEO_EXITBUTTON]->textureID = LoadTGA("Image//calibri.tga");
...
void SceneMainMenu::RenderMenu()
{
	modelStack.PushMatrix();
	
	modelStack.Translate(0.f, 20.f, -40.f);
	modelStack.Scale(15.f, 15.f, 0.f);
	RenderMesh(meshList[GEO_MENU], false, false);

	modelStack.PushMatrix();
	modelStack.Translate(-18.f, pos, 0.f);
	modelStack.Scale(0.1f, 0.1f, 0.f);
	RenderMesh(meshList[GEO_MENUCURSOR], false, false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0.f, 20.f, 10.f);
	modelStack.Scale(0.5f, 0.2f, 0.f);
	RenderMesh(meshList[GEO_GAMENAME], false, false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0.f, 5.f, 10.f);
	modelStack.Scale(0.4f, 0.1f, 0.f);
	RenderMesh(meshList[GEO_PLAYBUTTON], false, false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0.f, -5.f, 10.f);
	modelStack.Scale(0.4f, 0.1f, 0.f);
	RenderMesh(meshList[GEO_SETTINGSBUTTON], false, false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0.f, -15.f, 10.f);
	modelStack.Scale(0.4f, 0.1f, 0.f);
	RenderMesh(meshList[GEO_EXITBUTTON], false, false);
	modelStack.PopMatrix();
...
void SceneMainMenu::CheckSquareCollision()
{
...
void SceneMainMenu::RenderMesh(Mesh * mesh, bool enableLight, bool hasCollision)
{
	if (hasCollision)
	{
...
void SceneMainMenu::RenderFramerate(Mesh * mesh, Color color, float size, float x, float y)
{
